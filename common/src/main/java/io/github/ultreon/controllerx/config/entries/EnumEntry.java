...
public class EnumEntry<T extends Enum<T>> extends ConfigEntry<T> {
    ...
    public AbstractWidget createButton(Config options, int x, int y, int width) {
        CycleButton<T> cycleButton = CycleButton.<T>builder(o -> Component.nullToEmpty(this.get().name())).withValues(clazz.getEnumConstants()).withInitialValue(this.get()).displayOnlyValue().create(x, y, width, 20, Component.nullToEmpty("Value"), (cycleButton1, object) -> {
            this.set(cycleButton1.getValue());
            cycleButton1.setMessage(Component.nullToEmpty(this.get().name()));
        });
