...
import dev.ultreon.controllerx.api.input.dyn.*;
...
public sealed class ControllerAction<T extends Enum<T> & IControllerInterDynamic<?>> implements IControllerAction<T> permits ControllerAction.Axis, ControllerAction.Button, ControllerAction.Joystick, ControllerAction.Nulled, ControllerAction.Trigger {
    ...
    public static <T extends Enum<T> & IControllerInterDynamic<?>> ControllerAction<T> create(Class<T> type, String text) {
    ...
    public Vector2f get2DValue() {
        return getMapping().asVec2().get(tmp);
    }
    ...
    public ConfigEntry<T> createEntry(Config config, ControllerMapping<T> mapping, String id, Component description) {
        return entry = createEntry0(config, mapping, id, description);
    }

    @Override
    public @NotNull ControllerAction<T> nulled() {
        if (nullAction == null) {
            nullAction = new Nulled();
        }
    ...
    public T fromString(String text) {
        return defaultValue.fromName(text).as(defaultValue);
    }

    public boolean cooldown(int millis) {
        if (lastClick + millis < System.currentTimeMillis() || isJustPressed()) {
