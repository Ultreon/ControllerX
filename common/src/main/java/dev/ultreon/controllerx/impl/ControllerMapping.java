...
public final class ControllerMapping<T extends Enum<T> & IControllerInterDynamic<?>> implements IControllerMapping<T> {
    ...
    public boolean isVisible() {
        EventResult eventResult = event.invoker().onVisible(visible);
    ...
    @Override
    public @NotNull ControllerAction<T> getActionDirect() {
        return action;
    }
    ...
    @Override
    public Icon getIcon() {
        return action.getMapping().getIcon();
    }
    ...
    public ConfigEntry<?> createEntry(Config config) {
        ResourceLocation id = config.getContext().getLocation();
